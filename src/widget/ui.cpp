...
void ui_compute_matrices(UIState& state, int width, int height) {
    ...
    state.view_pos = state.camera_pos;
    glm::mat4 t = glm::translate(glm::mat4(1.0f), state.model_dir);
...
void ui_draw(UIState& state) {
    ...
    ImGui::SliderFloat3("Camera Position", &state.camera_pos.x, -20.0f, 20.0f);
    ...
    ImGui::Separator();
    ImGui::SliderFloat3("Model Pos", &state.model_dir.x, -10.0f, 10.0f);
    ImGui::SliderFloat3("Model Rot", &state.model_rot.x, -180.0f, 180.0f);
    ImGui::SliderFloat("Model Scale", &state.model_scale, 0.1f, 5.0f);
    ImGui::Separator();
    ...
    ImGui::Separator();
    ImGui::SliderFloat("Cube Size", &state.cube_size, 0.2f, 5.0f);
    ImGui::SliderFloat3("Cube Pos", &state.cube_pos.x, -10.0f, 10.0f);
    if (ImGui::Button("Create Cube")) state.create_cube = true;
    ImGui::Separator();
    ImGui::DragFloat("outline width", &state.outlinewidth, 0.0001f, 0.0f, 0.001f, "%.4f");
    ImGui::End();
}
